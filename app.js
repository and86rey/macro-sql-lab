...
async function fetchAllWorldBankData(indicator, country) {
    ...
    try {
        do {
            const url = `https://api.worldbank.org{country}/indicator/${indicator}?format=json&page=${page}&per_page=100`;
            ...
            const validObs = json[1]
                ...
                .map(obs => ({ date: parseInt(obs.date), value: parseFloat(obs.value) }));
...
function renderChart(data, kpiDefinitions) {
    ...
    const datasets = kpiDefinitions.map((kpi, i) => ({
        ...
        tension: 0.3,
        pointRadius: 2,
        spanGaps: true // Skips null values instead of plotting as zero
    }));
    ...
    chart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: { 
                legend: { display: true, labels: { color: '#888', font: { family: 'monospace' } } }
            },
            scales: {
...
/** 
 * MINIMAL HTML REQUIREMENT:
 * <div id="status-indicator"></div>
 * <div id="insight-box"></div>
 * <pre id="sql-display" style="background:#000; color:#0f0; padding:10px;"></pre>
 * <div style="height:400px"><canvas id="main-chart"></canvas></div>
 * <table><thead id="table-head"></thead><tbody id="table-body"></tbody></table>
 * <button onclick="runScenario('growth')">Growth</button>
 * <button onclick="runScenario('volatility')">Volatility</button>
 * <button onclick="runScenario('smoothing')">Smoothing</button>
 */
